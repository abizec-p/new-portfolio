const projects = [
  {
    id: 1,
    img: "/projectImg/portfolio-website.png",
    title: "Portfolio 2024",
    shortDesc:
      "A collection of visually-driven projects highlighting my skills in graphic and web design, with a focus on clean layouts, brand identity, and user experience.",
    tags: ["Html", "Css", "Reactjs"],
    website: "https://abishekportfolionew.netlify.app/",
  },
  {
    id: 2,
    img: "/projectImg/portfolio2025.png",
    title: "Portfolio 2025",
    shortDesc:
      "A showcase of my web development journey, featuring responsive, user-focused projects that blend clean design with functional performance.",
    tags: ["Html", "Css", "Reactjs"],
    website: "https://abishekpariyar.netlify.app/",
  },
  {
    id: 3,
    img: "/projectImg/PCA-website.png",
    title: "PCA Website",
    shortDesc:
      "A focused, high-converting page designed to showcase services, build trust, and drive client inquiries through clear CTAs and clean visuals.",
    tags: ["Html", "Css", "Reactjs"],
    website: "https://pariyarcreativeagency.netlify.app/",
  },
  {
    id: 4,
    img: "/projectImg/2broshvac.png",
    title: "2Bros HVAC - Website",
    shortDesc:
      "A responsive, user-friendly site showcasing HVAC services, founder insights, and easy contact options — built to highlight trust, expertise, and year-round comfort.",
    tags: ["Html", "Css", "Reactjs"],
    website: "https://2broshvac.ca/",
  },
  {
    id: 5,
    img: "/projectImg/calculator.png",
    title: "Calculatemate - Calculator app",
    shortDesc:
      "A responsive, user-friendly calculator giving 6 types of different calculator.",
    tags: ["Html", "Css", "Reactjs"],
    website: "https://calculatemate.netlify.app/",
  },
  {
    id: 6,
    img: "/projectImg/expensetrackerImg.png",
    title: "Ex-Track - Easy Expense Tracker App",
    shortDesc:
      "Developed a small-scale full-stack expense management application using Express.js for the backend. The app allows users to record and categorize income and expenses, displaying total balances, spending summaries, and insights such as the highest spending category. Data is dynamically rendered with real-time updates for a smooth and interactive experience..",
    tags: [
      "Html",
      "Css",
      "Reactjs",
      "Express.js",
      "Node.js",
      "FramerMotion",
      "Mongodb",
    ],
    website: "https://expentrackapp.netlify.app/",
  },
];
export default projects;
